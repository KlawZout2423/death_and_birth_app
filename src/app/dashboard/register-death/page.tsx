...
export default function AdminRegisterDeathPage() {
    ...
    try {
      const res = await fetch("/api/death-registration", {
        ...
        body: JSON.stringify({
          ...
          deathRegionId: data.deathRegionId || undefined,
          causeOfDeath: data.causeOfDeath,
          nationalId: undefined,
          informantName: data.informantName,
          informantRelation: data.informantRelation || undefined,
          contactNumber: data.contactNumber,
          doctorReportUrl: undefined,
          documents: [],
        }),
      });

      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body.message || "Failed to submit death record");

      setMessage("Death record submitted for verification.");
      ...
      setError(e.message ?? "Failed to submit death record");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <DashboardLayout role="admin" userName="Admin">
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          <h1 className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">Register Death (Registrar General)</h1>
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
          <DeathForm onSubmit={onSubmit} isLoading={submitting} regionOptions={regions} />
        </div>
      </div>
    </DashboardLayout>
  );
